...
class hCI(CIWavefunction):
    ...
    ***Reference: Hierarchy Configuration Interaction: Combining Seniority Number
    ...
    def assign_hci_pattern(self, hci_pattern):
        if hci_pattern not in ['pos_diag' , 'neg_diag' , 'vch' , 'hch', 'fci']:
            raise TypeError("hci version must be provided either `pos_diag`, `neg_diag`,`vch' , `hch` or `fci`")
        else:
